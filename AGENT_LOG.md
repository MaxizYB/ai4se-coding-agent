...
- **任务边界重构**：复测发现仅拦截写入仍会让模型先展示“我来修复”的散文，并在拒绝后自动继续推理。`ce6d5d6` 为 ChatRunner 增加确定性的 `inspect/explain/operate/change/demo/task` 请求模式和动作权限矩阵；只读/解释请求只允许读取类动作，越权动作在展示散文前拦截并立即把控制权还给用户。系统提示现在接收宿主确定的请求模式，未知请求默认只读；新增问题式询问与显式修复请求的分类回归。离线套件仍为 `242 passed, 2 deselected`，`ruff` 通过。
- **语言一致性**：中文输入却得到英文总结是系统提示未指定输出语言导致的产品缺陷。`783b557` 从最后一条真实用户消息确定中英文语言，并把该值作为权威 `Response language` 传给 Chat 上下文；所有用户可见 prose 必须使用该语言，动作协议保留英文。新增中文请求的上下文与输出回归，离线套件 `243 passed, 2 deselected`，`ruff` 通过。
- **通用任务边界纠偏**：复测“仓库有什么”总想修 `foo.py`、而“帮我做一个小游戏”被拒绝，暴露先前词表式意图分类把通用 harness 收缩成了样例规则。根因是 Chat 系统提示将 `examples/demo` 的真实 `foo.py` 减法修复作为精确协议示例，模型读到同一 fixture 后被该模板污染。`45dbb5a` 删除词表权限矩阵，恢复自然语言任务能力；提示改为“当前用户请求是唯一任务边界”，并把所有协议样例替换为与 demo 无关的模块/配置示例。写入仍走既有 diff/HITL 统一治理。新增只读巡检和“做小游戏”任务回归，离线套件 `244 passed, 2 deselected`，`ruff` 通过。
